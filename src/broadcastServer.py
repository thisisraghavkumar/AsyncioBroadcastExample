...
async def handleNewConnection(reader, writer):
    role = await reader.read(7)
    role = role.decode()
    if role == "listen":
        listeningConnections.append(writer)
        print("Listener added, Total listeners =",len(listeningConnections))
    elif role == "write":
        writingConnections.append(reader)
        print("Writer added, Total writers =",len(writingConnections))
    else:
        writer.write(b'Invalid Role')
        writer.close()


async def writeToListener(writer, data):
    writer.write(data)
    await writer.drain()


async def listenFromWriter(reader, outputQueue):
    readData = await reader.read()
    message = readData.decode()
    while message != EOB and message != "overnout":
...
async def writeToAllListeners(data, listeningConnections):
    listOfListeners = [asyncio.create_task(
        writeToListener(r, data)) for r in listeningConnections]
    await asyncio.gather(*listOfListeners)


async def listenFromAllWriters(writingConnections, outputQueue):
    listOfWriters = [asyncio.create_task(listenFromWriter(
        w, outputQueue)) for w in writingConnections]
    await asyncio.gather(*listOfWriters, return_exceptions=True)


async def closeListener(writer):
    await writer.close()


async def closeAllListeners(listeningConnections):
    listOfListeners = [asyncio.create_task(
        closeListener(r)) for r in listeningConnections]
    await asyncio.gather(*listOfListeners, return_exceptions=True)


async def writeHelloToAllListeners(n, listeners, server):
    i = 1
    while i <= n:
        print(i, ":", len(listeningConnections), "listeners")
        message = f'Hello World {i}'
        await writeToAllListeners(message.encode(), listeners)
        await asyncio.sleep(5)
        i += 1
    print("Sending End of Brodcast to all listeners...")
    await writeToAllListeners(EOB.encode(), listeners)
    print("Closng all writers to the listeners...")
    await closeAllListeners(listeners)
    print("Closing the server...")
    server.close()


async def writeMessageInQueueToAllListeners(outputQueue, listeners, server):
    newMessage = await outputQueue.get()
    while newMessage != EOB:
        await writeToAllListeners(newMessage.encode())
        newMessage = await outputQueue.get()
    print("Received end of broadcast signal...")
    print("Sending End of Brodcast to all listeners...")
    await writeToAllListeners(EOB.encode(), listeners)
    print("Closing all writers to the listeners...")
    await closeAllListeners(listeners)
    print("Closing the server...")
    server.close()

async def main(listeningConnections):
    server = await asyncio.start_server(handleNewConnection, '127.0.0.1', 8888)
    #asyncio.create_task(writeHelloToAllListeners(
    #    10, listeningConnections, server))
    asyncio.create_task(writeMessageInQueueToAllListeners(messageQueue, listeningConnections, server))
    async with server:
        await server.serve_forever()

try:
    asyncio.run(main(listeningConnections))
except asyncio.CancelledError:
